...
LOGFILE = mylogbase+"/gdesktop-cert-renew-"+gemini_util.SLICENAME+"_"+LOCALTIME+".log"
...
for my_manager in managers:
	...
	for node in pruned_GN_Nodes:
		...
		gemini_util.installLAMPCert(node,keyfile,cert_file,cmd+add_cmd,LOGFILE,debug)
	
	for node in pruned_MP_Nodes:
		gemini_util.installLAMPCert(node,keyfile,cert_file,cmd,LOGFILE,debug)
