...
# RSpec Namespaces
RSPEC_NS = ""
RSPEC_NS_01 = "http://protogeni.net/resources/rspec/0.1"
RSPEC_NS_02 = "http://protogeni.net/resources/rspec/0.2"
RSPEC_NS_2 = "http://www.protogeni.net/resources/rspec/2"
...
def manifest_v2_to_unis(manifest, slice_id):
    ...
    for rsnode in rsnodes:
        ...
        rsnodehosts = get_qualified_xml_children(rsnode, RSPEC_NS, 'host')
        for rsnodehost in rsnodehosts:
            hostname = rsnodehost.getAttribute('name').lower()
        ...
        NODE_IGNORED_ATTRS = ('client_id', 'sshdport')
        ...
        for rsiface in rsinterfaces:
            iface_virtual_id = rsiface.getAttribute('client_id').split(':')[-1].lower()
            unis_id = create_urn(domain=slice_id, node=node_virtual_id,
            ...
            unis_port.appendChild(
                create_text_element(unis_dom, UNIS_NS, "name", iface_component_id.split(':')[-1]))

            mac = rsiface.getAttribute('mac_address')
            if mac:
                unis_port.appendChild(
                    create_text_element(unis_dom, UNIS_NS, 'address', mac,
                                        attributes=(('type', 'mac'),) ))
            
            rsifaceips = get_qualified_xml_children(rsiface, RSPEC_NS, 'ip')
            for rsifaceip in rsifaceips:
                ip = rsifaceip.getAttribute('address')
                iptype = rsifaceip.getAttribute('type')
                if ip:
                    unis_port.appendChild(
                        create_text_element(unis_dom, UNIS_NS, 'address', ip,
                                            attributes=(('type', iptype),) ))
            ...
            for i in range(rsiface.attributes.length):
                ...
                if attr.prefix and attr.prefix != 'xmlns':
                    unis_port_pgeni_properties.setAttribute("xmlns:%s" % attr.prefix, 
                                                            attr.namespaceURI)

            INTERFACE_IGNORED_CHILDREN = ((RSPEC_NS, 'ip'),)
            clone_children(rsiface, unis_port_pgeni_properties, INTERFACE_IGNORED_CHILDREN)
        ...
        if rslamp_config:
            ...
            # This should be done as a XPath query..
            rsgmn = get_unique_xml_child(rsnode, GEMINI_NS, 'node')
            rsgmn = get_unique_xml_child(rsgmn, GEMINI_NS, 'services')
            rsgmn = get_unique_xml_child(rsgmn, GEMINI_NS, 'active')
            if rsgmn and rsgmn.getAttribute('install') == 'yes':
                unis_node_psconfig_properties = unis_dom.createElementNS(PSCONFIG_NS,
                                                                         "nodeProperties")
                unis_node_properties_bag.appendChild(unis_node_psconfig_properties)

        # All the other child nodes of rspec node we clone into the pgeni extension
        NODE_IGNORED_CHILDREN = ((LAMP_NS, 'config'), (RSPEC_NS, 'interface'), (RSPEC_NS, 'host'))
    ...
    for rslink in rslinks:
        link_virtual_id = rslink.getAttribute('client_id').lower()
        ...
        for i in range(rslink.attributes.length):
            ...
            if attr.prefix and attr.prefix != 'xmlns':
                unis_link_pgeni_properties.setAttribute("xmlns:%s" % attr.prefix, 
                                                        attr.namespaceURI)
...
def main(argv=None):
    ...
    try:
        ...
        global RSPEC_NS
        RSPEC_NS = ns
        if ns == RSPEC_NS_01 or ns == RSPEC_NS_02:
            unis_dom = manifest_to_unis(manifest_dom, slice_id)
        if ns == RSPEC_NS_2 or ns == RSPEC_NS_3:
            unis_dom = manifest_v2_to_unis(manifest_dom, slice_id)
        else:
            print "Manifest has unknown default namespace"
            return
...
def get_unique_xml_child(element, ns, name):
    # Allows some sort of "chaining"
    if element is None:
        return None
    
    # XXX: Maybe namespace could be None?
    assert ns != None and name != None
